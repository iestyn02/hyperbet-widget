...
**Scrollable viewport instead of manual pagination:** I would have liked to ditch the prev/next buttons entirely and just let the user scroll through their bets inside a fixed-height container, loading more rows as they approach the bottom. The tricky part is figuring out the right approach without breaking the hypermedia model. HTMX's `intersect` trigger is probably the right entry point: drop a sentinel row at the bottom of the table, and when it scrolls into view fire a request for the next batch and swap it in. The bigger problem is pagination strategy. The current offset approach (`?page=2`) falls apart here because new bets arrive at the top every 10s, which shifts everything down and causes duplicates or gaps mid-scroll. Cursor-based pagination would fix that: send the ID of the last visible row as `?after=<id>` and let the server return everything older than that, regardless of what's arrived since. The remaining issue is that you lose URL bookmarkability entirely since there's no clean way to represent "scrolled to row 47" in a query param. That tension between scroll-based UX and the stateless hypermedia model is roughly where I'd be stuck.
